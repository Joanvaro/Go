...
func fillingLoggerStruct(events []Event) ([]Logger) {
    ...
    for _, record := range events {
        ...
        for _, item := range record.Items {
            key := GetItemNameFromItemID(item.Id)
            
            for _, itemData := range item.Value {
                loggerData := LoggerItem {
                    ItemValue:  itemData.Value,
                    ItemTime:   itemData.Time,
                }
                recordStream.Items[key] = append(recordStream.Items[key], loggerData)
            }
        }
        loggerData = append(loggerData, recordStream)
    }
    return loggerData
}
...
func main() {
    ...
    data := fillingLoggerStruct(events)
